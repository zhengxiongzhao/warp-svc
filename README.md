...
Run Cloudflare WARP client as a proxy server in Docker.

This Docker image packages the official Cloudflare WARP client for Linux and provides a vproxy (Rust) powered proxy server (`vproxy run auto`, single-port auto protocol detection), usable in:
...
🔀 **多协议支持** - vproxy (Rust) `run auto` 单端口自动识别协议，无需区分入口
...
### Environment Variables
    ...
    environment:
      - BIND_ADDR=::          # 代理监听地址 (vproxy auto 自动识别协议)，默认 :: 双栈
      - BIND_PORT=1080        # 代理监听端口
      - ENABLE_IPV6=1         # Enable IPv6 routing and IPv6 egress, default 1
      - SOCKS_USER=admin      # 代理认证用户名（HTTP/SOCKS5 均生效）
      - SOCKS_PASS=123456     # 代理认证密码
      - LOG_LEVEL=error       # vproxy 日志级别: trace/debug/info/warn/error，默认 error
      # - WARP_PROXY=http://127.0.0.1:7890   # 注册走 HTTP/SOCKS5 代理（绕过数据中心 IP 注册限流）
...
IPv6 support is enabled by default. The container will preserve the IPv6 address generated by WARP, route both `0.0.0.0/0` and `::/0` through WireGuard, and listen on `[::]` for dual-stack proxy access (vproxy 会将 `::` 规范化为 `[::]`).
...
Verify that WARP is active and working (supporting HTTP, HTTPS, and SOCKS5 on a single port):
